...
const DoctorDetails = [
  ...
  {
    id: "2",
    name: "Dr. A-02130938-Pavan Kumar",
    experience: "8",
  ...
  {
    id: "3",
    name: "Dr. A-02130940-Vamshi Krishna",
    experience: "15",
    speciality: "Gastroenterologist",
    city: "Shimla",
  ...
  {
    id: "4",
    name: "Dr. A-02191280-Krishna Chaitanya",
    experience: "10",
    speciality: "Dentist",
    city: "Bhopal",
...
type ScreenStyles = {
  ...
  resultsButton: ViewStyle;
  resultsButtonText: TextStyle;
  ...
  resultsCountBadge: ViewStyle;
  resultsCountText: TextStyle;
  emptyContainer: ViewStyle;
  emptyText: TextStyle;
  paginationContainer: ViewStyle;
  pageButton: ViewStyle;
  pageText: TextStyle;
  bookingModalContent: ViewStyle;
  dateButton: ViewStyle;
  dateText: TextStyle;
  dayText: TextStyle;
  timeSlotButton: ViewStyle;
  bookingConfirmation: ViewStyle;
};
...
const getStyles = (colors: typeof Colors.dark): ScreenStyles =>
  StyleSheet.create({
    ...
    listContainer: { paddingHorizontal: 16, paddingTop: 8, paddingBottom: 80,marginTop:25,marginBottom:0 },
    doctorCard: {
      backgroundColor: colors.card,
      borderRadius: 16,
      marginBottom: 16,
...
const DoctorCard = ({
  ...
  <View style={styles.doctorCard}>
    <View style={styles.cardBody}>
      <Image
        source={{ uri: doctor.doctorPhoto || defaultUserImage }}
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.metaRow}>
          ...
          {doctor.rating && (
            <StarRating rating={doctor.rating} styles={styles} />
          )}
        </View>
        <View style={styles.metaItem}>
          <MapPin size={14} color={Colors.dark.textSecondary} />
          <Text style={styles.metaText}>{doctor.city || "N/A"}</Text>
        </View>
      </View>
    </View>
      ...
      <View style={styles.modalContainer}>
        ...
        {bookingStatus === "booked" ? (
          <View style={styles.bookingConfirmation}>
            ...
            <Text style={styles.metaText}>
              In Progress - Redirecting to payment...
            </Text>
          </View>
        ) : (
          <>
              ...
              <View style={styles.bookingModalContent}>
                        ...
                        >
                          ...
                          <Text
                            style={[
                              ...
                              selectedDate?.toDateString() ===
                              date.toDateString()
                                ? (styles.selectedOptionText as TextStyle)
                                : undefined,
                            ]}
                          >
                            {date.getDate()}
                    ...
                    {TIME_SLOTS.map((time) => (
                      <TouchableOpacity
                        ...
                        style={[
                          ...
                          selectedTime === time && styles.selectedOptionButton,
                        ]}
                ...
                <View style={styles.filterSection}>
                  ...
                  <TextInput
                    style={[
                      styles.searchInput,
                      { height: 100, textAlignVertical: "top", padding: 12 },
                    ]}
              ...
              >
                {bookingStatus === "booking" ? (
                  ...
                  <Text style={styles.resultsButtonText}>
                    Complete Booking & Pay ₹{doctor.consultationFee}
                  </Text>
                )}
              </TouchableOpacity>
            </View>
...
export default function FindDoctorScreen() {
  ...
  const applyFilters = (docs: Doctor[], filters: FilterState) => {
    return docs.filter((doc) => {
      ...
      const specSearchMatch = doc.speciality
        ?.toLowerCase()
        .includes(searchLower);
      const specialtyFilterMatch =
        filters.selectedSpecialties.length === 0 ||
        filters.selectedSpecialties.includes(doc.speciality!);
      const ratingFilterMatch =
        filters.selectedRatings.length === 0 ||
        filters.selectedRatings.some((r) => doc.rating! >= r);
      const locationFilterMatch =
        filters.selectedLocations.length === 0 ||
        filters.selectedLocations.includes(doc.city!);
      const feeFilterMatch =
        filters.selectedFee === null ||
        doc.consultationFee! <= filters.selectedFee;
      const expFilterMatch =
        filters.selectedExperience === null ||
        getExperienceRange(doc.experience!) === filters.selectedExperience;
      const langFilterMatch =
        filters.selectedLanguages.length === 0 ||
        filters.selectedLanguages.every((lang) =>
          doc.languages?.includes(lang)
        );

      return (
        (nameMatch || specSearchMatch) &&
        specialtyFilterMatch &&
        ratingFilterMatch &&
        locationFilterMatch &&
        feeFilterMatch &&
        expFilterMatch &&
  ...
  const filteredDoctors = useMemo(
    () => applyFilters(allDoctors, filterState),
    [allDoctors, filterState]
  );
  const tempFilteredDoctors = useMemo(
    () => applyFilters(allDoctors, tempFilterState),
    [allDoctors, tempFilterState]
  );
  ...
  const applyFiltersAndClose = () => {
    dispatch({
      ...
      payload: {
        field: "selectedSpecialties",
        value: tempFilterState.selectedSpecialties,
      },
    });
    dispatch({
      type: ACTIONS.SET_FIELD,
      payload: {
        field: "selectedRatings",
        value: tempFilterState.selectedRatings,
      },
    });
    dispatch({
      type: ACTIONS.SET_FIELD,
      payload: {
        ...
        value: tempFilterState.selectedLocations,
      },
    });
    dispatch({
      type: ACTIONS.SET_FIELD,
      payload: { field: "selectedFee", value: tempFilterState.selectedFee },
    });
    dispatch({
      type: ACTIONS.SET_FIELD,
      payload: {
        field: "selectedExperience",
        value: tempFilterState.selectedExperience,
      },
    });
    dispatch({
      type: ACTIONS.SET_FIELD,
      payload: {
        ...
        value: tempFilterState.selectedLanguages,
      },
    });
    setCurrentPage(1);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <FlatList 
        ...
        renderItem={({ item }) => (
          <DoctorCard doctor={item} styles={styles} onBook={handleBookNow} />
        )}
            ...
            <View style={[styles.paginationContainer]}>
              ...
              >
                <ChevronRight
                  size={20}
                  color={
                    currentPage === totalPages ? colors.border : colors.text
                  }
                />
              </TouchableOpacity>
            </View>
      ...
      >
        <View style={[styles.modalContainer]} >
              ...
              <View style={styles.optionsContainer}>
                {SPECIALTIES.map((spec) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optionButton,
                      tempFilterState.selectedSpecialties.includes(spec) &&
                        styles.selectedOptionButton,
                    ]}
                    onPress={() =>
                      handleTempDispatch({
                        type: ACTIONS.TOGGLE_ARRAY_ITEM,
                        payload: { field: "selectedSpecialties", value: spec },
                      })
                    }
                  >
                    <Text
              ...
              <View style={styles.optionsContainer}>
                {LOCATIONS.map((loc) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optionButton,
                      tempFilterState.selectedLocations.includes(loc) &&
                        styles.selectedOptionButton,
                    ]}
                    onPress={() =>
                      handleTempDispatch({
                        type: ACTIONS.TOGGLE_ARRAY_ITEM,
                        payload: { field: "selectedLocations", value: loc },
                      })
                    }
                  >
                    <Text
              ...
              <View style={styles.optionsContainer}>
                {FEES.map((fee) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optionButton,
                      tempFilterState.selectedFee === fee &&
                        styles.selectedOptionButton,
                    ]}
                    onPress={() =>
                      handleTempDispatch({
                        ...
                        payload: {
                          field: "selectedFee",
                          value:
                            tempFilterState.selectedFee === fee ? null : fee,
                        },
                      })
                    }
                  >
              ...
              <View style={styles.optionsContainer}>
                {EXPERIENCE.map((exp) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optionButton,
                      tempFilterState.selectedExperience === exp &&
                        styles.selectedOptionButton,
                    ]}
                    onPress={() =>
                      handleTempDispatch({
              ...
              <View style={styles.optionsContainer}>
                {RATINGS.map((rating) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optionButton,
                      tempFilterState.selectedRatings.includes(rating) &&
                        styles.selectedOptionButton,
                    ]}
                    onPress={() =>
                      handleTempDispatch({
                        type: ACTIONS.TOGGLE_ARRAY_ITEM,
                        payload: { field: "selectedRatings", value: rating },
                      })
                    }
                  >
                    <View
              ...
              <View style={styles.optionsContainer}>
                {LANGUAGES.map((lang) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.optionButton,
                      tempFilterState.selectedLanguages.includes(lang) &&
                        styles.selectedOptionButton,
                    ]}
                    onPress={() =>
                      handleTempDispatch({
                        type: ACTIONS.TOGGLE_ARRAY_ITEM,
                        payload: { field: "selectedLanguages", value: lang },
                      })
                    }
                  >
                    <Text
          ...
          <View style={styles.modalFooter}>
            <TouchableOpacity
              style={{ ...styles.resultsButton, backgroundColor: colors.card }}
              onPress={handleClearFilters}
            >
              <Text style={{ ...styles.resultsButtonText, color: colors.text }}>
                Clear All
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            onChangeText={(value) =>
              dispatch({
                type: ACTIONS.SET_FIELD,
                payload: { field: "searchQuery", value },
              })
            }
          />
        </View>
